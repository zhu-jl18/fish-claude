...
- 当前公开说明里覆盖 `codex`、`claude`、`gemini` 三类后端，并强调统一通过 `code-dispatcher` 入口调度

## 当前可关注内容

- `dev` / `wave` / `code-council` 等 workflow skill
- `github-issue-pr-flow`、`pr-review-reply`、`cc-cx-review-loop` 这类自主交付与 review 流程
- `install.py`、`uninstall.py`、`prompts/`、`skills/` 这些安装与运行时资产

## 待补

- 安装与接入方式
- 和本仓库 `skills/` / `sub-agents/` / `agent-instructions/` 的关系
- 适合场景与使用边界
