...
function runGitApply(packageRoot: string, mode: Exclude<Mode, "status">, quiet = false) {
...
function formatFailureDetails(result: { stderr: string; stdout: string }): string {
...
function logContext(mode: string, ompEntry: string, packageRoot: string): void {
	console.log(`[omp-patch-codex-websearch-byok] mode=${mode}`);
	console.log(`[omp-patch-codex-websearch-byok] ompEntry=${ompEntry}`);
	console.log(`[omp-patch-codex-websearch-byok] packageRoot=${packageRoot}`);
	console.log(`[omp-patch-codex-websearch-byok] patch=${patchPath}`);
	console.log("");
}
...
function handleReverseMode(ompEntry: string, packageRoot: string): number {
	console.log("[omp-patch-codex-websearch-byok] Reversing patch...");
	const patchState = detectPatchState(packageRoot);
	logContext("reverse", ompEntry, packageRoot);

	if (patchState.state === "can-apply") {
		console.log("[omp-patch-codex-websearch-byok] Patch is not currently applied.");
		return EXIT_OK;
	}
...
function handleStatusMode(ompEntry: string, packageRoot: string): number {
	console.log("[omp-patch-codex-websearch-byok] Checking patch status...");
	const patchState = detectPatchState(packageRoot);
	logContext("status", ompEntry, packageRoot);

	if (patchState.state === "can-apply") {
		console.log("[omp-patch-codex-websearch-byok] Status: patch can be applied.");
		return EXIT_OK;
	}

	if (patchState.state === "already-applied") {
		console.log("[omp-patch-codex-websearch-byok] Status: patch is already applied.");
		return EXIT_OK;
	}

	console.log("[omp-patch-codex-websearch-byok] Status: patch needs manual rebase.");
...
function main(): void {
	...
	try {
		if (mode === "apply") {
			process.exit(handleApplyMode(ompEntry, packageRoot));
		} else if (mode === "reverse") {
			process.exit(handleReverseMode(ompEntry, packageRoot));
		} else if (mode === "status") {
			process.exit(handleStatusMode(ompEntry, packageRoot));
		} else if (mode === "check" || mode === "check-reverse") {
			const label = mode === "check" ? "Checking" : "Checking reverse";
			console.log(`[omp-patch-codex-websearch-byok] ${label}...`);
			...
			console.log("");
			console.log("[omp-patch-codex-websearch-byok] Done.");
