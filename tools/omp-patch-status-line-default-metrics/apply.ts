...
function logContext(mode: string, ompEntry: string, packageRoot: string): void {
	console.log(`[omp-patch-status-line-default-metrics] mode=${mode}`);
	console.log(`[omp-patch-status-line-default-metrics] ompEntry=${ompEntry}`);
	console.log(`[omp-patch-status-line-default-metrics] packageRoot=${packageRoot}`);
	console.log(`[omp-patch-status-line-default-metrics] patch=${patchPath}`);
	console.log("");
}
...
function handleApplyMode(ompEntry: string, packageRoot: string): number {
	console.log("[omp-patch-status-line-default-metrics] Applying patch...");
	const patchState = detectPatchState(packageRoot);
	logContext("apply", ompEntry, packageRoot);

	if (patchState.state === "already-applied") {
		console.log("[omp-patch-status-line-default-metrics] Patch is already applied.");
		return EXIT_OK;
	}
	...
	console.log("");
	console.log("[omp-patch-status-line-default-metrics] Patch applied.");
	return EXIT_OK;
}

function handleReverseMode(ompEntry: string, packageRoot: string): number {
	console.log("[omp-patch-status-line-default-metrics] Reversing patch...");
	const patchState = detectPatchState(packageRoot);
	logContext("reverse", ompEntry, packageRoot);

	if (patchState.state === "can-apply") {
		console.log("[omp-patch-status-line-default-metrics] Patch is not currently applied.");
		return EXIT_OK;
	}
	...
	console.log("");
	console.log("[omp-patch-status-line-default-metrics] Patch reversed.");
	return EXIT_OK;
}

function handleStatusMode(ompEntry: string, packageRoot: string): number {
	console.log("[omp-patch-status-line-default-metrics] Checking patch status...");
	const patchState = detectPatchState(packageRoot);
	logContext("status", ompEntry, packageRoot);

	if (patchState.state === "can-apply") {
		console.log("[omp-patch-status-line-default-metrics] Status: patch can be applied.");
		return EXIT_OK;
	}

	if (patchState.state === "already-applied") {
		console.log("[omp-patch-status-line-default-metrics] Status: patch is already applied.");
		return EXIT_OK;
	}

	console.log("[omp-patch-status-line-default-metrics] Status: patch needs manual rebase.");
...
function handleDirectCheckMode(
	...
	console.log(`[omp-patch-status-line-default-metrics] ${label}`);
	...
	console.log("");
	console.log("[omp-patch-status-line-default-metrics] Done.");
	return EXIT_OK;
}
...
function main(): void {
	...
	try {
		...
		console.error("");
		console.error(
			`[omp-patch-status-line-default-metrics] Failed: ${error instanceof Error ? error.message : String(error)}`,
		);
